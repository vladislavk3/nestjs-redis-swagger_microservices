...
export class PasswordUtil {
  ...
  genHash(plainPassword: string) {
    return new Promise(accept => {
      bcrypt.genSalt(this.saltRounds, (_, salt: string) => {
        bcrypt.hash(plainPassword, salt, (_, hash: string) => {
          accept(hash);
        });
      });
    });
  ...
  verifyHash(plainPassword: string, hash: string) {
    return new Promise(accept => {
      bcrypt.compare(plainPassword, hash, function(_, result) {
        accept(result);
      });
    });
  }
